...
function canvasToPDFCoords(canvasX, canvasY, canvasWidth, canvasHeight, pdfWidth, pdfHeight) {
...
async function renderPreviewPage(pageNum) {
    ...
    try {
        ...
        const thumbnail = Array.from(thumbnails).find(t => parseInt(t.dataset.pageNum) === pageNum);
...
function shareApp() { ShareModal.show(); }
function closeShareModal() { ShareModal.close(); }
function handleSocialShare(platform) { ShareModal.handleSocialShare(platform); }
function handleCopyLink() { ShareModal.handleCopyLink(); }
function handleEmailShare() { ShareModal.handleEmailShare(); }
...
function addOverlayGizmo(type, label, x, y, overlayIndex) {
    ...
    const bgRemovalBadge = (overlay && overlay.removeBackground)
        ...
        if (action === 'move') {
            ...
            if (isCorner) {
                ...
                if (resizeHandle === 'se') {
                    ...
                    scale = Math.max((initialWidth + deltaX) / initialWidth, (initialHeight + deltaY) / initialHeight);
            ...
            if (currentOverlay) {
                const updates = { width: newWidth, height: newHeight, x: newLeft, y: newTop };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    textColorPicker.on('change', (color) => {
        ...
        window.selectedTextColor = hexColor;
        UIControls.updateLayerColor(hexColor, false);
    });

    // Expose text color picker to window
    ...
    bgColorPicker.on('change', (color) => {
        ...
        window.selectedBgColor = hexColor;
        UIControls.updateLayerColor(hexColor, true);
    });

    // Expose background color picker to window
