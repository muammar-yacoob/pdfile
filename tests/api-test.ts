...
async function runAPITests() {
	...
	const { testPdfPath, testImagePath, testMergePdfPath } =
	...
	try {
		...
		await test('Reorder pages', async () => {
			...
			if (firstBox && secondBox) {
				...
				const confirmBtn = page.locator(
					'button.modal-btn-primary:has-text("Confirm")',
				);
		...
		await test('Add date overlay and export', async () => {
			...
			return (
				fileName.includes('exported.pdf') || fileName.includes('with_date.pdf')
			);
		});
		...
		await test('Add image overlay', async () => {
			...
			return (
				fileName.includes('exported.pdf') || fileName.includes('overlay.pdf')
			);
		});
		...
		await test('Add signature overlay', async () => {
			...
			return (
				fileName.includes('exported.pdf') || fileName.includes('signed.pdf')
			);
		});
