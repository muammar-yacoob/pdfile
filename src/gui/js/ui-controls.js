...
const UIControls = (() => {
    ...
    function showLayerEditor(overlay, index) {
        ...
        if (isText) {
            ...
            if (overlay.removeBackground && !overlay._processedImageData && window.BackgroundRemover) {
    ...
    function toggleLayerTransparency() {
        ...
        if (isText) {
            ...
            if (gizmo) {
                ...
                if (textEl) {
                    textEl.style.backgroundColor = showBg ? window.selectedBgColor : 'transparent';
                }
            }
        } else {
            // Update overlay state properly via AppState
    ...
    async function applyBackgroundRemovalToPreview(index, overlay) {
        ...
        try {
            ...
            const processedImage = await window.BackgroundRemover.removeBackground(overlay.imageData, {
                tolerance: 25,
                featherRadius: 3,
                edgeDetection: true
            });
...
const SelectionManager = (() => {
    function selectOverlay(index) {
        ...
        document.querySelectorAll('.overlay-gizmo').forEach(g => g.classList.remove('selected'));

        // Add selected class to the selected gizmo
        const selectedGizmo = document.querySelector(`.overlay-gizmo[data-overlay-index="${index}"]`);
        if (selectedGizmo) {
