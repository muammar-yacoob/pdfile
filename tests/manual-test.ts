...
async function runTests() {
  ...
  try {
    ...
    const lucideLoaded = await page.evaluate(() => typeof (window as any).lucide !== 'undefined');
    const pdfjsLoaded = await page.evaluate(() => typeof (window as any).pdfjsLib !== 'undefined');
    ...
    console.log(fileName?.includes('test.pdf') ? '✓' : '❌', 'File name displayed');
    ...
    const overlaysActive = await page.locator('#t-overlays').evaluate(el => el.classList.contains('active'));
    ...
    const textTabActive = await page.locator('#overlay-text').evaluate(el => el.classList.contains('active'));
    ...
    await page.waitForTimeout(500);
    const panelCollapsed = await page.locator('#thumbnailsPanel').evaluate(el => el.classList.contains('collapsed'));
    ...
    await page.waitForTimeout(500);
    const mergeActive = await page.locator('#t-merge').evaluate(el => el.classList.contains('active'));
