	...
	async function exportPDF() {
		...
		try {
			...
			const pageOrder = thumbnails.map((item, idx) => ({
				pageNum:
					Number.parseInt(item.dataset.originalPage) ||
			...
			const hasReordering = pageOrder.some(
				(item, idx) => item.pageNum !== idx + 1,
			);
			...
			const convertedOverlays = await Promise.all(
				pendingOverlays.map(async (overlay, idx) => {
					...
					const isTextOverlay =
						overlay.type === 'date' || overlay.type === 'text';
					...
					if (isTextOverlay) {
						...
						const textWidth = overlay.width
							? Math.max(10, overlay.width - 16)
							: 100;
						const textHeight = overlay.height
							? Math.max(10, overlay.height - 24)
							: 30;
						...
						const imgWidth = overlay.width
							? Math.max(10, overlay.width - 8)
							: 100;
						const imgHeight = overlay.height
							? Math.max(10, overlay.height - 16)
							: 100;
					...
					if (
						...
						(convertedWidth <= 10 || convertedHeight <= 10)
					) {
						console.warn(
							`Small ${overlay.type} dimensions: ${convertedWidth}x${convertedHeight} from gizmo ${overlay.width}x${overlay.height}`,
						);
					}
