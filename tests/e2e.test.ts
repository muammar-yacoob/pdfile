...
test.describe('PDFile GUI Tests', () => {
  test('should load the GUI without errors', async ({ page }) => {
    ...
    expect(errors.filter(e => !e.includes('Tracking Prevention'))).toHaveLength(0);
  });
  ...
  test('should load PDF preview', async ({ page }) => {
    ...
    const embedExists = await page.locator('embed[type="application/pdf"]').count();
  ...
  test('should toggle tools panel', async ({ page }) => {
    ...
    const isActive = await page.locator('#t-overlays').evaluate(el =>
  ...
  test('should switch overlay tabs', async ({ page }) => {
    ...
    const textTabActive = await page.locator('#overlay-text').evaluate(el =>
      el.classList.contains('active')
    );
  ...
  test('should collapse/expand pages panel', async ({ page }) => {
    ...
    const isCollapsed = await page.locator('#thumbnailsPanel').evaluate(el =>
  ...
  test('should test merge PDFs flow', async ({ page }) => {
    ...
    const mergeToolActive = await page.locator('#t-merge').evaluate(el =>
      el.classList.contains('active')
    );
    expect(mergeToolActive).toBeTruthy();

    // Verify merge buttons exist
    const addToEndBtn = await page.locator('button:has-text("Add to End")').count();
