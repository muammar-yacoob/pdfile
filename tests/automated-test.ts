...
async function runTests() {
  ...
  try {
    ...
    await test('Lucide icons loaded', async () => {
      return await page.evaluate(() => typeof (window as any).lucide !== 'undefined');
    });

    await test('PDF.js loaded', async () => {
      return await page.evaluate(() => typeof (window as any).pdfjsLib !== 'undefined');
    });
    ...
    await test('Toggle overlays tool', async () => {
      ...
      return await page.locator('#t-overlays').evaluate(el => el.classList.contains('active'));
    });

    await test('Switch to text tab', async () => {
      await page.locator('.tabs .tab:has-text("Text")').click();
      await page.waitForTimeout(200);
      return await page.locator('#overlay-text').evaluate(el => el.classList.contains('active'));
    });
    ...
    await test('Pages panel collapses', async () => {
      ...
      await page.waitForTimeout(300);
      const collapsed = await page.locator('#thumbnailsPanel').evaluate(el => el.classList.contains('collapsed'));
      await page.locator('.thumbnails-header .collapse-btn').click();
      await page.waitForTimeout(200);
      return collapsed;
    });
    ...
    await test('Merge tool opens', async () => {
      ...
      await page.waitForTimeout(300);
      return await page.locator('#t-merge').evaluate(el => el.classList.contains('active'));
    });
